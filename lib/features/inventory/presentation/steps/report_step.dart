...
class ReportStep extends ConsumerWidget {
  ...
  String _generateReport(
    ...
    for (final item in state.items) {
      ...
      final catName =
          category.name.isNotEmpty ? category.name : 'Без категории';
                ...
                try {
                  final pdfItems = state.items
                      .map((i) => {
                            'name': i.productName,
                            'remaining':
                                i.remaining == i.remaining.truncateToDouble()
                  ...
                  if (template != null) {
                    final usedColumns = template.columns
                        .where((c) => c.mappedField != TemplateField.notUsed)
                        .toList();
                    customHeaders =
                        usedColumns.map((c) => c.originalHeader).toList();
                    customFieldOrder =
                        usedColumns.map((c) => c.mappedField.name).toList();
                  }
