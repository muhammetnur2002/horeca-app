/// Данные и CustomPainter анимации заставки (звёзды, частицы, "падающие
/// звёзды", орбиты, планеты, логотип, текст). Вынесены из splash_screen.dart,
...
class Shoot {
  ...
  Shoot({required this.x, required this.y, required this.vx,
...
class GalaxyPainter extends CustomPainter {
  ...
  void _drawPlanets(Canvas canvas, double cx, double cy, double W, double H) {
    final p = eo(ph(0.3, 1.8));
    final planets = [
      PlanetDef(rx: W * 0.32, ry: H * 0.1, rot: -0.52,
  ...
  @override
  bool shouldRepaint(GalaxyPainter old) => true;
}

class PlanetDef {
  final double rx, ry, rot, ang, r;
  final Color c0, c1;
  const PlanetDef({required this.rx, required this.ry, required this.rot,
