        ...
        ..._manualWriteOffs.asMap().entries.map((e) {
          ...
          return Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 8),
              Text(m.unit, style: const TextStyle(fontSize: 12, color: AppColors.muted)),
              const SizedBox(width: 8),
  ...
  Widget _buildStep3(bool isDark, String currency) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _GlassCard(isDark: isDark, child: Column(
        ...
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _CashField(label: 'Начало смены',
              controller: _morningCashCtrl, isDark: isDark, currency: currency,
              onChanged: (_) => setState(() {}))),
          const SizedBox(width: 10),
          Expanded(child: _CashField(label: 'Конец смены',
...
class _PaymentRow extends StatelessWidget {
  ...
  const _PaymentRow({required this.icon, required this.label, required this.hint,
      required this.controller, required this.color, required this.isDark,
